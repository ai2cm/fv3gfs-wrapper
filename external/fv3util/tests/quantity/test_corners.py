...
def test_fill_scalar_corners_copies_from_halo(quantity, direction, tile_partitioner, rank, n_halo):
    quantity.data[:] = 0
    # put nans in corners
    ...
    quantity.view[:] = 2
    print(quantity.data)
    fv3util.fill_scalar_corners(quantity=quantity, direction=direction, tile_partitioner=tile_partitioner, rank=rank, n_halo=n_halo)
    print(quantity.data)
    assert quantity.np.sum(quantity.np.isnan(quantity.data)) == 0
    assert quantity.np.all(quantity.view[:] == 2)  # should be unchanged
    quantity.np.testing.assert_array_equal(quantity.view.southwest[-n_halo:0, -n_halo:0], 0)
    quantity.np.testing.assert_array_equal(quantity.view.southeast[0:n_halo, -n_halo:0], 0)
    quantity.np.testing.assert_array_equal(quantity.view.northwest[-n_halo:0, 0:n_halo], 0)
    quantity.np.testing.assert_array_equal(quantity.view.northeast[0:n_halo, 0:n_halo], 0)
