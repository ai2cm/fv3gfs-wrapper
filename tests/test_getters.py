...
def redirect_stdout(filename):
    ...
    def _redirect_stdout(to_file_descriptor):
        """Redirect stdout to the given file descriptor."""
        # Flush the C-level buffer stdout
        libc.fflush(c_stdout)
        # Flush and close sys.stdout - also closes the file descriptor (fd)
        sys.stdout.close()
        # Make original_stdout_file_descriptor point to the same file as to_file_descriptor
        os.dup2(to_file_descriptor, original_stdout_file_descriptor)
        # Create a new sys.stdout that points to the redirected fd
        sys.stdout = io.TextIOWrapper(os.fdopen(original_stdout_file_descriptor, 'wb'))

    # Save a copy of the original stdout fd in saved_stdout_file_descriptor
    saved_stdout_file_descriptor = os.dup(original_stdout_file_descriptor)
    try:
        # Create a temporary file and redirect stdout to it
        temporary_stdout = tempfile.TemporaryFile(mode='w+b')
        _redirect_stdout(temporary_stdout.fileno())
        # Yield to caller, then redirect stdout back to the saved fd
        yield
        _redirect_stdout(saved_stdout_file_descriptor)
        # Copy contents of temporary file to the given stream
        temporary_stdout.flush()
        temporary_stdout.seek(0, io.SEEK_SET)
        stream.write(temporary_stdout.read())
    finally:
