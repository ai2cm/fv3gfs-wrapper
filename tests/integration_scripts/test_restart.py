...
if __name__ == '__main__':
    ...
    for i in range(num_steps):
        ...
        fv3gfs.step_physics()
    fv3gfs.save_intermediate_restart(label='checkpoint')
    comm.barrier()
    checkpoint_data = fv3gfs.get_restart_data(os.path.join(os.getcwd(), 'RESTART'), label='checkpoint')
    for i in range(num_steps):
        print(f'Step {i}')
        fv3gfs.step_dynamics()
        fv3gfs.step_physics()
    fv3gfs.save_intermediate_restart(label='first_time')
    comm.barrier()
    first_time_data = fv3gfs.get_restart_data(os.path.join(os.getcwd(), 'RESTART'), label='first_time')
    fv3gfs.set_state(checkpoint_data)
    fv3gfs.save_intermediate_restart(label='after_checkpoint_reset')
    comm.barrier()
    logging.info('comparing checkpoint to restart output after resetting to checkpoint')
    checkpoint_reset_data = fv3gfs.get_restart_data(os.path.join(os.getcwd(), 'RESTART'), label='after_checkpoint_reset')
    test_data_equal(
    ...
    for i in range(num_steps):
        ...
        fv3gfs.step_dynamics()
        fv3gfs.step_physics()
    fv3gfs.save_intermediate_restart(label='second_time')
    comm.barrier()
    second_time_data = fv3gfs.get_restart_data(os.path.join(os.getcwd(), 'RESTART'), label='second_time')
